...
public class NioEndpoint extends AbstractJsseEndpoint<NioChannel,SocketChannel> {
    ...
    /**
     * TODO: 开启nio 的 endpoint， 创建acceptor线程 ，以及 poller 线程
     * TODO: 干了三件比较重要的事，1.创建处理客户端请求的线程池，2. 创建了Poller线程 3. 开启了Acceptor线程
    ...
    public void startInternal() throws Exception {
        ...
        if (!running) {
            ...
            if (getExecutor() == null) {
                // TODO: 创建了一个线程池，用于处理客户端请求 worker线程池，就是在这儿创建的
                createExecutor();
            }
            ...
            // Start poller thread
            // TODO: 创建poller线程，它就一个线程，用于处理socket连接
            poller = new Poller();
    ...
    protected boolean setSocketOptions(SocketChannel socket) {
        ...
        try {
            ...
            if (nioChannels != null) {
                // TODO: 尝试从缓存里面弹出来一个channel （NioChannel） NioChannel主要封装了socketChannel...
                channel = nioChannels.pop();
            }
            // TODO: 如果在nioChannels中没取到，这时候就需要创建一个了
            if (channel == null) {
            ...
            // TODO: 创建了一个nioSocketWrapper NioSocket包装器
            NioSocketWrapper newWrapper = new NioSocketWrapper(channel, this);
            // TODO: channel重新设置，避免它是脏的，重新设置了socket
            channel.reset(socket, newWrapper);
            // TODO: 缓存到connections中
            connections.put(socket, newWrapper);
            socketWrapper = newWrapper;

            // Set socket properties
            // Disable blocking, polling will be used
            // TODO: 配置非阻塞，nio里面的那一套
            ...
            // TODO: 将socketWrapper以及channel注册到poller中
    ...
    /**
     * TODO: 用于监听events，添加events，然后处理产生事件的连接
     * Poller class.
     */
    public class Poller implements Runnable {
        ...
        /**
         * TODO: 初始化的时候，直接就Open了一个selector
         * @throws IOException
         */
        ...
        /**
         * TODO: 如果存在事件，会进行标记存在事件，然后判断感兴趣的事件，注册到当前poller的selector上
        ...
        public void register(final NioChannel socket, final NioSocketWrapper socketWrapper) {
            // TODO: 注册感兴趣的事件 读事件，读取客户端传过来的数据
            socketWrapper.interestOps(SelectionKey.OP_READ);//this is what OP_REGISTER turns into.
            PollerEvent event = null;
            // TODO: 从缓存里头 弹出来个event
            if (eventCache != null) {
                event = eventCache.pop();
            }
            // TODO: 如果event等于空，就进行创建，PollerEvent
            if (event == null) {
                event = new PollerEvent(socket, OP_REGISTER);
            } else {
                // TODO: 否则就进行reset 重新设置
                event.reset(socket, OP_REGISTER);
            }
            // TODO: 添加event到events中
            addEvent(event);
        }
        ...
        public void run() {
            ...
            while (true) {
                ...
                try {
                    // TODO: poller线程运行，消费events，如果没有关闭
                    if (!close) {
                        // TODO: 第一步先看看有没有事件, 如果存在事件，判断事件的感兴趣状态，然后注册事件到selector上面
                        hasEvents = events();

                        /* ---------------- 进行selector挑选 -------------- */
                        // TODO: wakeupCounter 这个值默认是0，在addEvent的时候，会进行+1，这就意味着存在event.
                        if (wakeupCounter.getAndSet(-1) > 0) {
                            // TODO: 如果进到这里了，就采用非阻塞，立马select，意味着还需要去处理其他事情，所以就不阻塞了
                ...
                while (iterator != null && iterator.hasNext()) {
                    ...
                    // cancelledKey()
                    // TODO: 这时候直接移除就ok了，当它为null的时候，表示其他线程已经取消了这个key
                    if (socketWrapper == null) {
                        iterator.remove();
                    } else {
                        // TODO: 移除，然后进行处理 key processKey
                        iterator.remove();
                        // TODO: 很重要的一个方法，去处理selectionKey
        ...
        protected void processKey(SelectionKey sk, NioSocketWrapper socketWrapper) {
            try {
                // TODO: 如果close掉了，就取消掉这个selectionKey
                if (close) {
                    cancelledKey(sk, socketWrapper);
                    // TODO: 如果这个selectionKey是有效的，并且socketWrapper不为空
                        ...
                        if (socketWrapper.getSendfileData() != null) {
                            ...
                            if (sk.isReadable()) {
                                if (socketWrapper.readOperation != null) {
                                    ...
                                    // TODO: 处理socket, processSocket 这个方法很重要
    ...
    protected class SocketProcessor extends SocketProcessorBase<NioChannel> {
        ...
        protected void doRun() {
            ...
            try {
                int handshake = -1;
                try {
                    // TODO: 如果已经握手完成，对于SSL连接，tomcat需要进行握手操作，这里如果是NIOChannel的话，直接返回true.
                ...
                // TODO: 说明握手成功了，NioChannel 会直接返回0
                if (handshake == 0) {
